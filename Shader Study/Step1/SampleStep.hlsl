...
struct PixelShaderInput
{
    ...
    float4 pos : SV_Position;   // 이게 uv역할 해야됨
};

// 시간변수 없어도, 수동으로 시간을 넣자. ==> 시간이 문제가 아닌게 화면이 검정색으로 나옴.
float4 main(PixelShaderInput input):SV_Target{

    return mainImage(input.color, input.pos.xy);
}