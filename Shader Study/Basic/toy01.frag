...
float utri(float x){return abs(2.*fract(x*1.5-.5)-1.);}
...
float time = u_time*0.1;
  p.xy*=mat2(cos(-u_time),sin(-u_time),-sin(-u_time),cos(-u_time));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	mat3 rot = rotationXY( vec2( u_time, u_time*0.32 ) );
